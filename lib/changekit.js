...
function init(mysqlDatabase) {
    if (!mysqlDatabase.hasOwnProperty("query")) {
        throw "ChangeKit requires a MySQL database as input. See 'mysql' on npm";
    }

    mysqlDatabase.query({
        sql: "CREATE TABLE IF NOT EXIST `changekit` ( `resource` TEXT NOT NULL, `value` TEXT NOT NULL, `lastUpdated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ) ENGINE=MEMORY"
    }, function (error) {
        if (error) {
            throw error;
        }
